...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const arg2 = Math.sqrt(x1 ** 2 + y1 ** 2) * Math.sqrt(x2 ** 2 + y2 ** 2);
  // const cosTheta = (x1 * x2 + y1 * y2) / (Math.sqrt(x1 ** 2 + y1 ** 2) * Math.sqrt(x2 ** 2 + y2 ** 2));
  const cosTheta = arg1 / arg2;
...
function isPowerOfTwo(num) {
  ...
  const isLogNumInteger = Number.isInteger(logNum);
  return isLogNumInteger;
}
...
function isNumber(number) {
  ...
  const isNANNumber = Number.isNaN(number);
  const finiteNumber = Number.isFinite(number);
  if (typeNumber === 'number') {
    if (!isNANNumber && finiteNumber) {
      return true;
    }
  }
  return false;
...
function isSafeInteger(number) {
  const result = Number.isSafeInteger(number);
  return result;
}
...
function getSumOfNumbers(x1, x2, x3) {
  const sum3 = x1 + x2 + x3;
  const result = Number(sum3.toFixed(2));
  return result;
}
